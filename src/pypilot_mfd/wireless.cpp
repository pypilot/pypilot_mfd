...
struct sensor_transmitter_t {
    sensor_transmitter_t() : position(SECONDARY) {}
...
template <struct T>
struct transmitters {
    std::map<uint64_t, T> transmitters;
    static uint64_t cur_primary;
    static uint32_t cur_primary_time;

    T& update(uint8_t mac[6], bool &primary,  float dir=0) {
        uint64_t mac_int = mac_as_int(mac);
        uint32_t t = millis();
        primary = cur_primary == mac_int;
        T &wt = transmitters[mac];
...
static transmitters<wind_transmitter_t> air_transmitters;
static transmitters<wind_transmitter_t> water_transmitters;
static transmitters<wind_transmitter_t> lightning_uv_transmitters;
...
static void DataRecvWind(struct esp_now_data_t &data) {
    ...
    if (packet->period > 100)  // period of 100uS cups is about 193 knots for 100 rotations/s
        ...
        knots = 25800.0 / packet->period;  // corrected from preliminary calibration
    else
        knots = 0;
...
static void DataRecvLightningUV(struct esp_now_data_t &data)
{
    ...
    bool primary;
    lighting_uv_transmitter_t &wt = lightning_uv_transmitters.update(data.mac, primary);
